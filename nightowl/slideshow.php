<?php $display_slideshow = get_post_meta($post->ID, 'classic_slideshow', true); // Values: empty string/enable/disable ?>
<?php if($display_slideshow != 'disable'){ ?>
		<script type="text/javascript">
			...
			function scrollSlides(){
				if(jQuery('#slide_' + current_slide).hasClass('fit')){
					...
					//}else{
						...
						//}
						//przesun o: (n-1)slajdow.width() + (window width - ostatni element)
			...
			function sliderAnimationsFlow(){
				...
				jQuery("#slide_"+current_slide).find('.button').animate({ 'opacity': 1, 'margin-top' : button_margin }, 700);
				var amap = {'opacity': 1};
				amap[padding_type] = img_margin;
				jQuery("#slide_"+current_slide).find('.slide-img').animate(amap, 300);
			}
		...
		jQuery(document).ready(function(){
			...
			// Create controls (keyboard, mouse click)
			// Mouse click
			if(Modernizr.touch){
